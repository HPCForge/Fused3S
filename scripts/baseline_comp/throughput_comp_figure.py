...
file_name = f"{results_path}/baseline_comp_kernel_only_throughput_all_all_all_{gpu_name}.csv"
try:
    ...
    for dataset in datasets:
        ...
        # Store the throughput values for each algorithm
        for alg in algs_original:
            ...
            if not np.isnan(value):
                value = value / 1e12
            # Store throughput values in TFLOPS
...
# Sort datasets by scale to make the line chart more readable
...
# Customize the plot
plt.ylabel('Throughput (TFLOPS)', fontsize=font_size)
...
# Using linear scale for y-axis
# plt.yscale('log') - removed as per request
...
# Add data labels for the highest throughput algorithm at each dataset
for i, dataset in enumerate(datasets):
    # Find the maximum throughput for this dataset
    max_value = -float('inf')
    max_alg = None
    for alg in algs_original:
    ...
    if max_alg:
        ...
        plt.annotate(f'{max_value:.1f}T',  # Format in TFLOPS
                    xy=(i, max_value),
...
plt.title(f'Throughput Comparison on {gpu_name} GPU', fontsize=font_size + 4)

# Save the figure
plt.savefig(f'{results_path}/throughput_comparison_{gpu_name}.png', dpi=300, bbox_inches='tight')
plt.close()

# Print summary stats
print("\nThroughput Comparisons - Geometric Means:")
...
for alg, geomean in sorted_algs:
    ...
    print(f"{display_name}: {geomean:.2f} TFLOPS")
